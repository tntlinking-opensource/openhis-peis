<template>
  <el-select value-key v-model="bindValue" :multiple="multiple" :placeholder="selectData.placeholder || '请选择...'"
    :size="selectSize" class="search-select" :popper-class="multiple ? 'search-popper multiple' : 'search-popper'"
    :style="{ width: inputWidth }" :clearable="!hideClear" @change="change" :loading="loading" :disabled="selectDisabled"
    @visible-change="visibleChange">
    <div class="data-search">
      <span class="search-title" v-if="selectData.inputTitle">{{ selectData.inputTitle }}</span>
      <el-input class="search-input" v-model="inputData" clearable size="mini"
        :placeholder="selectData.inputPlaceholder || '请输入'" />
    ...
    <template v-if="selectData.optionFlag == 'id'">
      <el-option v-for="item in optionData" :key="item.id" :label="item.label" :value="item.id"
        :disabled="item.value == 'empty'">
        <div class="data-option">
          <div v-if="selectData.key" class="value" :style="!selectData.thirdName ? 'width:35%' : ''">{{ item.value }}
          </div>
    ...
    <template v-else>
      <el-option v-for="item in optionData" :key="item.id" :label="item.label" :value="item.label"
        :disabled="item.value == 'empty'">
        <div class="data-option">
          <div v-if="selectData.key" class="value" :style="!selectData.thirdName ? 'width:35%' : ''">{{ item.value }}
          </div>
    ...
    <div class="data-pagination">
      <el-pagination small :total="total" background :page-size="20" layout="prev, pager, next"
        ...
        if (this.selectData.changeId) {
          ...
          this.bindValue.forEach((el) => {
            for (let i = 0; i < this.data.length; i++) {
              if (el == this.data[i].name) {
                array.push(this.data[i].id)
              }
            }
          })
