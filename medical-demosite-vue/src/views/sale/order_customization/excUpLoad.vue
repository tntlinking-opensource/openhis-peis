<!-- 公共池-b上传名单 麦沃德科技开发人:矢北 -->
<template>
  <el-dialog title="客户导入" :visible.sync="open" width="446px" append-to-body :close-on-click-modal="false"
    ...
    <el-form :inline="true" v-loading="loading" element-loading-text="导入中" label-position="top">
      <el-form-item label="请选择客户模板文件" style="width: 100%;">
        <file-upload ref="uploadFile" :uploadData="uploadData" @uploadFinish="uploadFinish"
          uploadWidth="100%"></file-upload>
      </el-form-item>
    </el-form>
...
<script>
import { upLoadList } from '@/api/sale/order_customization.js'
export default {
  data() {
    return {
      id: null,
  ...
  methods: {
    ...
    uploadFinish(value, res) {
      ...
      if (value == 1) {
        ...
        upLoadList({ id: this.id, fchange: this.fchange, path: res.data.urlList.join(), }).then(() => {
    ...
    // 提交
    submitForm() {
      this.$confirm('此操作将永久删除先前上传材料, 是否继续?', '提示', {
