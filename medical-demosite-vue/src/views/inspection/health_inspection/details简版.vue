...
export default {
  ...
  methods: {
    ...
    openAiAnalysis() {
      ...
      const promptText = `请作为资深总检专家对以下客户的体检数据进行全面分析：
顾客信息：
...
体检数据和结果：
${resultsTable}
请基于循证医学原则，提供专业、准确、实用的总检结论，不要重复结论，简单明了。
请严格按照以下json格式要求返回分析结果并确保格式完整准确可解析：
{
  "issuesAndConclusions": {
    "title": "主要健康问题汇总及健康建议",
    "textarea": "只需要给出以下格式：序号.问题标题：结论和描述；日常干预和治疗及健康建议（需要关注是否需要定期复查，如果需要复查就根据症状严重程度给出复查建议，如果症状并不是很严重复查周期尽可能以年或者半年位单位，健康建议需重点关注一些微量元素摄入建议）（输出内容要求如下：一个问题对应一个结论和描述一个日常干预和治疗及健康建议，只汇总异常指标，特别注意：结论和描述不要太冗余（只给出总结描述和结论即可）、千万不要输出健康的和未见明显异常的检查结果、如果症状不严重不要出现会引起客户恐慌的表述，需要有textarea的格式方便写入PDF中）"
  }
}
特别注意：textarea中的主要健康问题汇总主要来自体检数据和结果中的“总检结论词汇总”的内容，再参考”总检综述“给出更加准确和全面的主要健康问题汇总及健康建议。
`;
