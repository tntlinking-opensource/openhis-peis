...
<template>
  <div class="app-container flex-direction-column">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['basis:occupation:conclusion:add']">添加 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['basis:occupation:conclusion:edit']">编辑 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete" v-hasPermi="['basis:occupation:conclusion:remove']">删除 </el-button>
      </el-col>
      ...
      <el-table v-loading="loading" :data="symptomList" @selection-change="handleSelectionChange" stripe border height="100%">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200">
          <template slot-scope="scope">
            <el-button size="mini" type="text" style="color: #ffba00" @click="handleUpdate(scope.row)" v-hasPermi="['basis:occupation:conclusion:edit']">编辑</el-button>
            <el-button size="mini" type="text" style="color: #ff2727" @click="handleDelete(scope.row)" v-hasPermi="['basis:occupation:conclusion:remove']">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        serialNo: [{ required: true, message: '序列号不能为空', trigger: 'change' }],
        occupationSummary: [{ required: true, message: '检查结论名称不能为空', trigger: 'change' }],
        printForShort: [{ required: true, message: '打印中文简称不能为空', trigger: 'change' }],
      },
    }
  },
  created() {
