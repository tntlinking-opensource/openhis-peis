...
<template>
  <div class="app-container flex-direction-column">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain size="mini" :disabled="multiple" icon="el-icon-circle-check" @click="handlePass" v-hasPermi="['report:reportReview:healthFirst:pass']">审核通过 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain size="mini" :disabled="single" icon="el-icon-circle-close" @click="handleUnpass" v-hasPermi="['report:reportReview:healthFirst:unpass']">审核不通过 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain size="mini" :disabled="multiple" icon="el-icon-bottom-left" @click="handleUnaudit" v-hasPermi="['report:reportReview:healthFirst:unaudit']">反审 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" plain size="mini" icon="el-icon-plus" @click="handleAudit" v-hasPermi="['report:reportReview:healthFirst:audit']">批量通过 </el-button>
      </el-col>
    </el-row>
    <!-- 表格 -->
    <div class="table-box">
      <el-table ref="tableList" border v-loading="loading" :data="tableList" height="100%" stripe @selection-change="handleSelectionChange" @row-click="rowClick">
        ...
        <el-table-column label="操作" fixed="right" width="200" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button size="mini" type="text" style="color: #0059ff" @click="handlePass(scope.row)" v-hasPermi="['report:reportReview:healthFirst:pass']">通过</el-button>
            <el-button size="mini" type="text" style="color: #ff2727" @click="handleUnpass(scope.row)" v-hasPermi="['report:reportReview:healthFirst:unpass']">不通过</el-button>
            <el-button size="mini" type="text" style="color: #ffba00" @click="handleUnaudit(scope.row)" v-hasPermi="['report:reportReview:healthFirst:unaudit']">反审</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
import { getListData, healthFirstPass, healthFirstUnaudit } from '@/api/report/report_review/health_first'
import { getPatientData } from '@/api/report/report_print/health_report.js'
  ...
  methods: {
    ...
    handlePass(row) {
      if (row.id) {
        if (row.status && (row.status == 3 || row.status > 4)) {
          this.$alert('体检号:' + row.patientcode + '已通过审核，操作失败！', '提示', {
        ...
        for (var i in this.rows) {
          if (this.rows[i].status && (this.rows[i].status == 3 || this.rows[i].status > 4)) {
            this.$alert('体检号:' + this.rows[i].patientcode + '已通过审核，操作失败！', '提示', {
              confirmButtonText: '确定',
              type: 'warning',
            })
            return
      ...
      this.$modal
        .confirm('是否确认通过该数据项？', '提示')
        .then(function () {
          return healthFirstPass(ids)
        })
        .then((res) => {
          // if (row.id) {
          //   getPatientData(row.patientcode).then((res) => {
          //     if (res.data[0].isjj == 1) {
          //       this.$alert(`体检号 ${row.patientcode} 的顾客已置为加急,请加急处理!`)
          //     }
          //   })
          // } else {
          //   getPatientData(this.rows.map((item) => item.patientcode).join(',')).then((res) => {
          //     let code = []
          //     res.data.forEach(el=>{
          //       if(el.isjj == 1){
          //         code.push(el.patientcode)
          //       }
          //     })
          //     if (code.length) {
          //       console.log(123);
          //       this.$alert(`体检号 ${code.join(',')} 的顾客已置为加急,请加急处理!`)
          //     }
          //   })
          // }
          this.getList()
    ...
    handleUnpass(row) {
      ...
      if (row.status) {
        if (row.status != 4) {
          if (row.status == 3) {
            this.$alert('体检号:' + row.patientcode + '已通过审核，请先反审再操作！', '提示', {
              confirmButtonText: '确定',
              type: 'warning',
            })
            return
          } else if (row.status < 2) {
            this.$alert('体检号:' + row.patientcode + '的报告尚未打印，操作失败！', '提示', {
              confirmButtonText: '确定',
              type: 'warning',
            })
            return
          } else if (row.status > 4) {
            this.$alert('体检号:' + row.patientcode + '不在一审范围内，操作失败！', '提示', {
              confirmButtonText: '确定',
              type: 'warning',
            })
            return
        ...
        this.$alert('体检号:' + row.patientcode + '状态异常，操作失败！', '提示', {
          confirmButtonText: '确定',
          type: 'warning',
        })
        return
    ...
    handleUnaudit(row) {
      if (row.id) {
        if (row.status < 3 || row.status == 4) {
          this.$alert('体检号:' + row.patientcode + '尚未通过审核，操作失败！', '提示', {
            confirmButtonText: '确定',
            type: 'warning',
          })
          return
        ...
        for (var i in this.rows) {
          if (this.rows[i].status) {
            if (this.rows[i].status < 3 || this.rows[i].status == 4) {
              this.$alert('体检号:' + this.rows[i].patientcode + '尚未通过审核，操作失败！', '提示', {
                confirmButtonText: '确定',
                type: 'warning',
              })
              return
            } else if (this.rows[i].status > 3) {
              this.$alert('体检号:' + this.rows[i].patientcode + '不在一审范围内，操作失败！', '提示', {
                confirmButtonText: '确定',
                type: 'warning',
              })
              return
            ...
            this.$alert('体检号:' + this.rows[i].patientcode + '状态异常，操作失败！', '提示', {
              confirmButtonText: '确定',
              type: 'warning',
            })
            return
      ...
      const ids = row.id || this.ids
      this.$modal
        .confirm('是否确认反审该数据项？', '提示')
        .then(function () {
          return healthFirstUnaudit(ids)
        })
        .then((res) => {
          this.getList()
