...
<template>
  <div class="app-container flex-direction-column">
    ...
    <el-form :model="queryParams" ref="queryForm" size="mini" :inline="true" class="no-margin-bottom">
      ...
      <el-form-item label="终审时间" prop="startTime">
        <el-date-picker v-model="queryParams.startTime" style="width: 180px" value-format="yyyy-MM-dd" type="date" placeholder="请选择时间"> </el-date-picker>
      </el-form-item>
      <el-form-item label="-" prop="endTime">
        <el-date-picker v-model="queryParams.endTime" style="width: 180px" value-format="yyyy-MM-dd" type="date" placeholder="请选择时间"> </el-date-picker>
      </el-form-item>
      ...
      <el-form-item label="审核状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态" clearable style="width: 180px">
          ...
          <el-option label="终审通过" :value="7" />
          <el-option label="终审未通过" :value="8" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain size="mini" :disabled="multiple" icon="el-icon-circle-check" @click="handlePass" v-hasPermi="['report:reportReview:healthLast:pass']">审核通过 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain size="mini" :disabled="single" icon="el-icon-circle-close" @click="handleUnpass" v-hasPermi="['report:reportReview:healthLast:unpass']">审核不通过 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain size="mini" :disabled="multiple" icon="el-icon-bottom-left" @click="handleUnaudit" v-hasPermi="['report:reportReview:healthLast:unaudit']">反审 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" plain size="mini" icon="el-icon-plus" @click="handleAudit" v-hasPermi="['report:reportReview:healthLast:audit']">批量通过 </el-button>
      </el-col>
    </el-row>
    <!-- 表格 -->
    <div class="table-box">
      <el-table ref="tableList" border v-loading="loading" :data="tableList" height="100%" stripe @selection-change="handleSelectionChange" @row-click="rowClick">
        ...
        <el-table-column label="总检时间" prop="datefinalexamed" min-width="160" align="center" show-overflow-tooltip />
        <el-table-column label="总检大夫" min-width="120" prop="doctorfinalNameR" align="center" show-overflow-tooltip />
        ...
        <el-table-column label="二审人" min-width="120" prop="secondMan" align="center" show-overflow-tooltip />
        <el-table-column label="二审时间" min-width="120" prop="secondTime" align="center" show-overflow-tooltip />
        <el-table-column label="终审人" min-width="120" prop="lastMan" align="center" show-overflow-tooltip />
        <el-table-column label="终审时间" min-width="120" prop="lastTime" align="center" show-overflow-tooltip />
        <el-table-column label="操作" fixed="right" width="200" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button size="mini" type="text" style="color: #0059ff" @click="handlePass(scope.row)" v-hasPermi="['basis:chargeCategory:edit']">通过</el-button>
            <el-button size="mini" type="text" style="color: #ff2727" @click="handleUnpass(scope.row)" v-hasPermi="['basis:chargeCategory:query']">不通过</el-button>
            <el-button size="mini" type="text" style="color: #ffba00" @click="handleUnaudit(scope.row)" v-hasPermi="['basis:chargeCategory:remove']">反审</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
import { getListData, healthLastPass, healthLastUnaudit } from '@/api/report/report_review/health_last'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getListData(queryParams)
        .then((response) => {
          ...
          this.loading = false
          if (this.queryParams.patientcode && this.total == 1) {
            this.queryParams.patientcode = this.tableList[0].patientcode
          }
          this.rows = []
        })
        .catch((error) => {
    ...
    handlePass(row) {
      if (row.id) {
        if (row.status && (row.status == 7 || row.status > 8)) {
        ...
        for (var i in this.rows) {
          if (this.rows[i].status && (this.rows[i].status == 7 || this.rows[i].status > 8)) {
    ...
    handleUnpass(row) {
      ...
      if (row.status) {
        if (row.status != 7 && row.status != 8) {
          if (row.status > 8) {
            this.$alert('体检号:' + row.patientcode + '不在二审范围内，操作失败！', '提醒', {
              confirmButtonText: '确定',
              type: 'warning',
            })
            return
      ...
      if (row.id) {
        ...
        for (var i in this.rows) {
          if (this.rows[i].status) {
            if (this.rows[i].status < 7 || this.rows[i].status == 8) {
              ...
              this.$alert('体检号:' + this.rows[i].patientcode + '不在二审范围内，操作失败！', '提醒', {
                confirmButtonText: '确定',
                type: 'warning',
              })
              return
    ...
    //批量通过
    handleAudit() {
      this.$refs.pltgItems.handleAudit(this.rows)
    },
  },
}
</script>
