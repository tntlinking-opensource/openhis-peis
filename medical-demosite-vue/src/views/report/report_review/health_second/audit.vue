...
<template>
  <el-dialog :title="title" :visible.sync="open" width="1200px" class="health-audit" append-to-body>
    <div class="flex-direction-column">
      <el-form ref="queryForm" :model="queryParams" :inline="true" @submit.native.prevent hide-required-asterisk size="mini">
        <el-form-item label="体检号:" prop="patientcode">
          <el-input ref="patientcode" v-model="queryParams.patientcode" placeholder="请输入体检号" clearable style="width: 200px" @keyup.enter.native="handleQuery" />
        </el-form-item>
...
<script>
import { getListData, healthSecondPass } from '@/api/report/report_review/health_second'
export default {
  data() {
    return {
      // 查询参数
  ...
  methods: {
    ...
    submitForm() {
      ...
      healthSecondPass(this.tableList.map((item) => item.id)).then((response) => {
