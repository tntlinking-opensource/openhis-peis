...
<script>
import { getListData, professionLastPass } from "@/api/report/jobreport_review/profession_last";
export default {
	components: {},
	props: [],
	data() {
	...
	methods: {
			...
			getListData(this.queryParams).then(response => {
				if (!response.data.records.length) {
					...
					if (this.tableList.map((item) => item.id).indexOf(response.data.records[0].id) == -1) {
						if (response.data.records[0].status == 7 || response.data.records[0].status > 8) {
		...
		submitForm() {
			...
			professionLastPass(this.tableList.map((item) => item.id)).then(response => {
