<!-- 健康复审-批量通过 开发人：麦沃德科技暴雨/半夏 -->
<template>
	<el-dialog :title="title" :visible.sync="open" width="1200px" class="profession-audit" append-to-body>
		<div class="flex-direction-column">
			<el-form ref="queryForm" :model="queryParams" :inline="true" @submit.native.prevent hide-required-asterisk>
				<el-form-item label="体检号:" prop="patientcode">
					<el-input v-model="queryParams.patientcode" placeholder="请输入体检号" clearable style="width: 200px"
				...
				<el-form-item>
					<el-row>
						<el-button type="primary" plain size="mini" icon="el-icon-plus" @click="handleQuery">查询
						</el-button>
			...
			<div class="table-box" style="min-height: 400px;">
				<el-table border v-loading="loading" :data="tableList" height="100%" stripe
					...
					<el-table-column label="总检时间" min-width="160" prop="datefinalexamed" align="center"
						show-overflow-tooltip />
					<el-table-column label="总检大夫" prop="doctorfinalNameR" min-width="120" align="center"
						show-overflow-tooltip />
...
<script>
import { getListData, professionSecondPass } from "@/api/report/jobreport_review/profession_second";
export default {
	components: {},
	props: [],
	data() {
	...
	methods: {
			...
			getListData(this.queryParams).then(response => {
				if (!response.data.records.length) {
					...
					if (this.tableList.map((item) => item.id).indexOf(response.data.records[0].id) == -1) {
						if (response.data.records[0].status == 5 || response.data.records[0].status > 6) {
		...
		submitForm() {
			...
			professionSecondPass(this.tableList.map((item) => item.id)).then(response => {
