    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain size="mini" :disabled="multiple" icon="el-icon-circle-check" @click="handlePass"
          v-hasPermi="['report:jobreportReview:professionSecond:pass']">审核通过
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain size="mini" :disabled="single" icon="el-icon-circle-close" @click="handleUnpass"
          v-hasPermi="['report:jobreportReview:professionSecond:unpass']">审核不通过
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain size="mini" :disabled="multiple" icon="el-icon-bottom-left"
          @click="handleUnaudit" v-hasPermi="['report:jobreportReview:professionSecond:unaudit']">反审
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" plain size="mini" icon="el-icon-plus" @click="handleAudit"
          v-hasPermi="['report:jobreportReview:professionSecond:audit']">批量通过
        </el-button>
      </el-col>
        ...
        <el-table-column label="操作" fixed="right" width="200" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button size="mini" type="text" style="color: #0059FF" @click="handlePass(scope.row)"
              v-hasPermi="['report:jobreportReview:professionSecond:pass']">通过</el-button>
            <el-button size="mini" type="text" style="color: #ff2727" @click="handleUnpass(scope.row)"
              v-hasPermi="['report:jobreportReview:professionSecond:unpass']">不通过</el-button>
            <el-button size="mini" type="text" style="color: #ffba00" @click="handleUnaudit(scope.row)"
              v-hasPermi="['report:jobreportReview:professionSecond:unaudit']">反审</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
import { getListData, professionSecondPass, professionSecondUnaudit } from "@/api/report/jobreport_review/profession_second";
import btgItems from './unpass'
import pltgItems from './audit'
export default {
  name: "Profession_Second",
  ...
  methods: {
    ...
    handlePass(row) {
      ...
      this.$modal
        .confirm('是否确认通过该数据项？')
        .then(function () {
          return professionSecondPass(ids);
        })
        .then((res) => {
          this.getList();
    ...
    handleUnaudit(row) {
      ...
      this.$modal
        .confirm('是否确认反审该数据项？')
        .then(function () {
          return professionSecondUnaudit(ids);
        })
        .then((res) => {
          this.getList();
