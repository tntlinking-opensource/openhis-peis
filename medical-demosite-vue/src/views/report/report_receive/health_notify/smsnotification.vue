<!-- 健康报告领取通知-短信通知 开发人：麦沃德科技暴雨、予安 -->
<template>
  <div class="add-container">
    <!-- 添加或修改对话框 -->
    <el-dialog :title="title" :visible.sync="open" width="790px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :inline="true" label-width="100px" hide-required-asterisk :destroy-on-close="true" :rules="rules">
        ...
        <el-form-item label="选择时间" prop="sendTime">
          <el-date-picker v-model="form.sendTime" type="datetime" placeholder="选择日期时间" style="width: 600px" :disabled="form.immediately" value-format="yyyy-MM-dd HH:mm:ss"> </el-date-picker>
        </el-form-item>
      </el-form>
      ...
      <div style="min-height: 300px">
        <el-form :inline="true" @submit.native.prevent>
          <el-form-item label="体检号" prop="patientcode">
            <el-input ref="patientcode" v-model="patientcode" placeholder="请输入体检号" clearable style="width: 150px" @keyup.enter.native="handleAdd" />
          </el-form-item>
          <el-form-item>
            <el-row :gutter="10" class="mb8">
              <el-col :span="1.5">
                <el-button type="primary" plain size="mini" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['report:reportReceive:healthNotify:add']" :disabled="!patientcode">添加 </el-button>
              </el-col>
              <el-col :span="1.5">
                <el-button type="danger" plain size="mini" icon="el-icon-delete" @click="handleremove" v-hasPermi="['report:reportReceive:healthNotify:remove']" :disabled="multiple">删除 </el-button>
              </el-col>
            </el-row>
          </el-form-item>
...
<script>
import { getMessageTypeData, searchCode, sendMsg } from '@/api/report/report_receive/health_notify.js'
export default {
  components: {},
  props: [],
  data() {
    return {
      ...
      open: false,
      rules: {
        messageName: [{ required: true, message: '请选择短消息名称', trigger: 'change' }],
      },
    }
  },
  methods: {
    ...
    handleAdd() {
      ...
      this.loading = true
      searchCode(this.patientcode)
        .then(({ data }) => {
          this.examList.unshift(data)
          this.$nextTick(() => {
            this.$refs.patientcode.focus()
            this.patientcode = ''
          })
