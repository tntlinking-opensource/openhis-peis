<!-- 健康报告交接 麦沃德科技 开发人:清风/暴雨 -->
<template>
  <div class="app-container flex-direction-column">
    <el-form :model="queryParams" ref="queryParams" size="small" :inline="true" class="no-margin-bottom">
      ...
      <el-form-item label="体检者姓名" prop="name">
        <el-input placeholder="请输入姓名" clearable v-model="queryParams.name" style="width: 150px" @keyup.enter.native="handleQuery"></el-input>
      </el-form-item>
      ...
      <el-form-item label="单位名称" prop="numorgresv">
        <el-input placeholder="请输入单位名称" clearable v-model="queryParams.numorgresv" style="width: 180px" @keyup.enter.native="handleQuery"></el-input>
      </el-form-item>
      ...
      <el-form-item label="交接人" prop="xsjlid">
        <el-input placeholder="请输入交接人输入码" clearable v-model="queryParams.joinPersonId" style="width: 160px" @keyup.enter.native="handleQuery"></el-input>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-circle-check" size="mini" @click="handerWindow()" v-hasPermi="['report:reportHandover:healthHandover:handover']">交接 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-circle-close" size="mini" @click="returnHanderWindow()" v-hasPermi="['report:reportHandover:healthHandover:unhandover']">反交接 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit-outline" size="mini" @click="editWindow()" v-hasPermi="['report:reportHandover:healthHandover:edit']" :disabled="multiple">编辑 </el-button>
      </el-col>
    </el-row>

    <div class="table-box" style="display: flex; flex-direction: row">
      <el-table ref="tableData" :data="tableData" style="display: inline-block; flex-grow: 1" v-loading="loading" border stripe height="100%" @selection-change="handleSelectionChange" @row-click="rowClick">
        ...
        <el-table-column prop="printTime" label="打印时间" min-width="120px" align="center" show-overflow-tooltip></el-table-column>
    ...
    <el-dialog title="编辑" :visible.sync="open" width="400px" style="min-height: 250px" append-to-body :close-on-click-modal="false">
      <el-form :inline="true" :model="editForm" ref="editForm" label-width="80px" style="max-width: 400px; margin-left: 0">
...
import { getChartData, getListData, getPatientData, getBoxData, saveEdit, JKexport } from '@/api/report/health_handover/health_handover'
  ...
  methods: {
    ...
    getList() {
      ...
      getListData(this.queryParams).then((response) => {
        ...
        if (this.queryParams.patientcode && this.tableData.length == 1) {
          this.queryParams.patientcode = this.tableData[0].patientcode
        }
        this.loading = false
      })
    },
    ...
    getChart() {
      getChartData().then((response) => {
        ...
        this.initEcharts() //当获取数据后再调用echarts图片
        this.loading = false
      })
    },
    // 搜索
    ...
    handleExports() {
      this.download(
        '/report/healthAssociate/exportStatistics',
        {
          ...this.queryParams,
        },
        `报告交接统计.xlsx`
    ...
    editWindow(row) {
      ...
      if (!this.single) {
        this.editForm.chestNum = this.rows[0].chestNum
        this.editForm.grantId = this.rows[0].grantId
      } else {
        this.editForm.chestNum = undefined
        this.editForm.grantId = undefined
      }
    },
    //操作编辑
    editWindowc(row) {
