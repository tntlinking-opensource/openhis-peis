...
<template>
  ...
  <el-dialog :title="title" :visible.sync="open" class="add-charge" width="792px" append-to-body :close-on-click-modal="false">
    <el-form ref="queryExamination" :model="queryExamination" :rules="rules" :inline="true" label-width="110px" hide-required-asterisk>
      ...
      <el-form-item>
        <el-button type="primary" plain size="mini" icon="el-icon-plus" @click="handleadd" v-hasPermi="['report:reportCollection:getReprot:add']">添加 </el-button>
        <el-button type="primary" plain size="mini" icon="el-icon-delete" @click="handleremove" :disabled="multiple" v-hasPermi="['report:reportCollection:getReprot:remove']">删除 </el-button>
        <el-button type="success" plain size="mini" icon="el-icon-edit" @click="handleSign" v-hasPermi="['report:reportCollection:getReprot:signature']">签名 </el-button>
      </el-form-item>
    </el-form>
    <el-table ref="examList" border :data="examList" stripe height="500" @selection-change="handleSelectionChange" @row-click="rowClick">
      ...
      <el-table-column label="体检者姓名" prop="patientname" align="center" width="90px" show-overflow-tooltip />
...
<script>
import { getBoxData, getReport, getSousData } from '@/api/report/report_collection/get_reprot'
  ...
  methods: {
    ...
    handleadd() {
      if (!this.queryExamination.patientCode && !this.queryExamination.patientName && !this.queryExamination.phone) {
        ...
        this.loading = true
        getSousData({ ...this.queryExamination, tjlx: 1 })
          .then((response) => {
            let error = ''
