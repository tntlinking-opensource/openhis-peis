  ...
  <div class="app-container flex-direction-column">
    <el-form :model="queryParams" ref="queryForm" size="mini" :inline="true" @submit.native.prevent class="no-margin-bottom">
      <el-form-item label="参数键名" prop="paramKey">
        <el-input style="width: 180px" v-model="queryParams.paramKey" placeholder="请输入参数键名" @keyup.enter.native="resetQuery" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button class="zk-btn-style1" icon="el-icon-refresh" size="mini" @click="resetQuery">搜索</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button class="zk-btn-style2" icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['applet:system_config:edit']">修改</el-button>
          </el-col>
        </el-row>
      </el-form-item>
    </el-form>
    <div class="table-box">
      <el-table ref="tableDate" border v-loading="loading" :data="tableDate" height="100%" @selection-change="handleSelectionChange" @row-click="rowClick">
        ...
        <el-table-column label="参数键名" align="center" prop="paramKey" show-overflow-tooltip />
        <el-table-column label="参数值" align="center" prop="paramValue" show-overflow-tooltip />
        <el-table-column label="备注" align="center" prop="remark" show-overflow-tooltip />
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['approval:system_config:edit']">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog :title="title" :visible.sync="open" width="1060px" class="sub-center report-setting" destroy-on-close append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline="true" hide-required-asterisk v-loading="loading">
        <el-form-item label="参数值" prop="paramValue">
          <el-input type="textarea" :rows="4" v-model="form.paramValue" placeholder="请输入参数值" clearable style="width: 810px" />
        </el-form-item>
...
<script>
import { getListApi, saOrUp, getDetailsApi, deleteApi } from '@/api/applet/system_config.js'
export default {
  name: 'System_config',
  data() {
    return {
      ...
      // 表单校验
      rules: {
        paramValue: [{ required: true, message: '参数值不能为空', trigger: 'change' }],
