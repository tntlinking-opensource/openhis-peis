...
<template>
  <div class="app-container flex-direction-column">
    ...
    <el-form size="small" :model="queryParams" ref="queryForm" :inline="true" class="no-margin-bottom">
      <el-form-item prop="fzx" label="分中心">
        <el-input style="width: 230px" v-model="queryParams.fzx" placeholder="请输入分中心"></el-input>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button class="zk-btn-style2" icon="el-icon-plus" size="mini" @click="handleAdd" v-hasPermi="['applet:center_list:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="zk-btn-style2" icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate" v-hasPermi="['applet:center_list:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button class="zk-btn-style2" icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleRemove" v-hasPermi="['applet:center_list:remove']">删除</el-button>
      </el-col>
    </el-row>
    <div class="table-box">
      <el-table ref="tableDate" border v-loading="loading" :data="tableDate" height="100%" @selection-change="handleSelectionChange" @row-click="rowClick">
        ...
        <el-table-column label="缩略图" align="center">
          <template slot-scope="scope">
            <el-image style="width: 60px; height: 60px" :src="imgPath + scope.row.picture" :preview-src-list="[imgPath + scope.row.picture]"> </el-image>
          </template>
        </el-table-column>
        ...
        <el-table-column label="状态" align="center">
          <template slot-scope="scope">
            <span style="color: green" v-if="scope.row.isShow == 1">启用</span>
            <span style="color: red" v-else-if="scope.row.isShow == 0">关闭</span>
          </template>
        </el-table-column>

        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['applet:center_list:edit']">修改</el-button>
            <el-button size="mini" type="text" icon="el-icon-delete" @click="handleRemove(scope.row)" v-hasPermi="['applet:center_list:remove']">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline="true" hide-required-asterisk v-loading="loading">
        ...
        <el-form-item label="图片" prop="picture" style="display: block">
          <div class="setimg">
            ...
            <el-image v-if="form.id && form.picture" :src="imgPath + form.picture" :preview-src-list="[imgPath + form.picture]" style="width: 100px; height: 100px"></el-image>
          </div>
        </el-form-item>
        <el-form-item label="地址" prop="address">
          <el-input v-model="form.address" placeholder="请输入地址" clearable style="width: 810px" />
        </el-form-item>
        <el-form-item label="经度" prop="lng">
          <el-input v-model="form.lng" placeholder="请输入经度" clearable style="width: 340px" />
        </el-form-item>
        <el-form-item label="纬度" prop="lat">
          <el-input v-model="form.lat" placeholder="请输入纬度" clearable style="width: 340px" />
        </el-form-item>
        <el-form-item label="状态" prop="isShow">
          <el-select v-model="form.isShow" placeholder="请选择状态" style="width: 340px">
            <el-option label="启用" :value="1" />
            <el-option label="关闭" :value="0" />
          </el-select>
        </el-form-item>
...
<script>
import { getListApi, saOrUp, getDetailsApi, deleteApi } from '@/api/applet/center_list.js'
...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        fzx: [{ required: true, message: '分中心不能为空', trigger: 'change' }],
      },
    }
  },
  created() {
