<!-- 上传图片 开发人：麦沃德科技半夏 -->
<template>
  <el-dialog title="传图" :visible.sync="open" width="464px" append-to-body :close-on-click-modal="false" :destroy-on-close="true">
    <el-form :model="form" ref="form" size="small" :inline="true" :rules="rules" hide-required-asterisk label-width="70px" v-loading="loading">
      ...
      <!-- <el-form-item label="上传结果" style="margin-right: 0">
        <el-input :value="uploadResult" readonly style="width: 420px" type="textarea" :rows="5"></el-input>
      </el-form-item> -->
      <el-form-item label="上传结果" style="margin-right: 0">
          <el-input
...
<script>
import { getKsList, upLoadFilePic } from '@/api/funcdept/section_list/dialog.js'
export default {
  data() {
    return {
      // 打开弹窗
  ...
  computed: {
    formattedUploadResult() {
      ...
      return this.uploadResult.replace(/\r\n|\n/g, '<br>');
    }
  },
  methods: {
    // 打开弹窗
