...
<script>
import { getCardData, handlePay, handleAdvancePay,getBranchData } from '@/api/reception/proceeds.js'
      ...
      this.$refs['queryForm'].validate((valid) => {
        if (valid) {
          ...
          .then(() => {
              ...
              //let queryParamsJsonString = JSON.stringify(this.queryParams);
              let queryParamsJsonString = JSON.stringify(queryParams);
              console.log("queryParamsJsonString的值",queryParamsJsonString);
              
              if (this.isPrePay) {
                handleAdvancePay(1,queryParamsJsonString)
                  .then(({ data }) => {
                    this.$modal.msgSuccess('保存成功')
                    this.loading = false
                    this.$emit('paySuccess', data.id, this.queryParams.cardId, data.version, '体检卡', 1, this.queryParams.jsf)
                    this.cancel()
                  })
                  .catch(() => {
                    this.loading = false
                ...
                handlePay(queryParams)
                  .then(({ data }) => {
                    this.$modal.msgSuccess('保存成功')
                    this.loading = false
                    this.$emit('paySuccess', data.id, this.queryParams.cardId, data.version, '体检卡')
                    this.cancel()
                  })
                  .catch(() => {
                    this.loading = false
