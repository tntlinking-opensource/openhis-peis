  ...
  <el-dialog title="会员卡积分调整" :visible.sync="open" width="750px" append-to-body :close-on-click-modal="false" destroy-on-close @close="cancel">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" label-width="90px" v-loading="loadingCard" :rules="rules" hide-required-asterisk>
      ...
      <el-form-item label="充值积分" prop="limit">
        <el-input type="number" v-model="queryParams.limit" :readonly="!isPrePay" style="width: 250px" @blur="changeLimit"> </el-input>
      </el-form-item>
    ...
    <!-- <el-table v-loading="loading" :data="tableData" stripe border height="400px">
      ...
      <el-table-column label="体检卡号" align="center" prop="cardno" show-overflow-tooltip min-width="100px" />
      <el-table-column label="付款金额" align="center" prop="money">
        <template slot-scope="scope">
          <el-input v-model="scope.row.money" controls-position="right" size="small" width="100%"> </el-input>
        </template>
      </el-table-column>
      <el-table-column label="备注" align="center" prop="memotext" show-overflow-tooltip min-width="260px"> </el-table-column>
      <el-table-column label="可用金额" align="center" prop="balanceLimit" show-overflow-tooltip />
    </el-table> -->
...
<script>
import { getMemberData, handlePay, handleAdvancePay } from '@/api/reception/proceeds.js'
...
export default {
  ...
  methods: {
    dialogShow(info, ids, version, isPrePay) {
      this.readOnly = false
      this.queryParams = {
      ...
      if (ids.id) {
        ...
        this.queryParams.cardno = ids.cardno
              ...
              if (this.isPrePay) {
                handleAdvancePay({
                  ...
                  .then(({ data }) => {
                    ...
                    this.loadingCard = false
                    this.$emit('paySuccess', data.id, this.queryParams.patientcardno, data.version, '会员积分', 1, this.queryParams.jsf)
                    this.cancel()
                  })
                  .catch(() => {
                    this.loadingCard = false
                ...
                handlePay({
                  ...
                  .then(({ data }) => {
                    this.$modal.msgSuccess('保存成功')
                    this.loadingCard = false
                    this.$emit('paySuccess', data.id, this.queryParams.patientcardno, data.version, '会员积分')
                    this.cancel()
                  })
                  .catch(() => {
                    this.loadingCard = false
