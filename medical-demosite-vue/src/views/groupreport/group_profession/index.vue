  ...
  <div class="app-container flex-direction-column">
    ...
    <el-form :model="queryParams" ref="queryForm" size="mini" :inline="true" class="no-margin-bottom">
      <el-form-item label="团体名称" prop="orgName">
        <input-select ref="orgName" :selectData="orgNameData" selectWidth="230" @change="orgNameDataChange"></input-select>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain size="mini" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['groupreport:groupProfession:add']">新增 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain size="mini" icon="el-icon-edit-outline" @click="handleUpdate" :disabled="isPass" v-hasPermi="['groupreport:groupProfession:edit']">编辑 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain size="mini" icon="el-icon-delete" @click="handleDelete" :disabled="isPass" v-hasPermi="['groupreport:groupProfession:remove']">删除 </el-button>
      </el-col>
      <el-col :span="2">
        <el-button type="warning" plain size="mini" icon="el-icon-plus" @click="handleJoiners" :disabled="isPass" v-hasPermi="['groupreport:groupProfession:Joiners']">加入人员 </el-button>
      </el-col>
    </el-row>
    <el-row :gutter="20" style="height: 90%">
        ...
        <div class="table-box">
          <el-table border id="setTable" ref="tableData" v-loading="loading" :data="tableList" height="100%" stripe @selection-change="handleSelectionChange" @row-click="rowClick">
            ...
            <el-table-column label="报告状态" prop="status" align="center" show-overflow-tooltip>
              <template slot-scope="scope">
                <span>{{ ['草稿', '提交', '通过', '不通过', '撤回', '主检未审', '主检开审', '主检已审'][scope.row.status] }}</span>
              </template>
            </el-table-column>
      ...
      <el-col :span="9" style="height: 100%" class="flex-direction-column">
        ...
        <el-row :gutter="10" class="mb8">
          ...
          <el-col :span="1.5">
            <el-button type="success" plain size="mini" icon="el-icon-refresh-right" @click="handlerefresh" v-hasPermi="['groupreport:groupProfession:refresh']" :disabled="this.ids.length == 0">刷新 </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="primary" plain size="mini" icon="el-icon-plus" @click="rightHandleAdd" v-hasPermi="['groupreport:groupProfession:add']" :disabled="this.ids.length == 0 || isPass">新增 </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain size="mini" icon="el-icon-delete" @click="rightHandleDelete" v-hasPermi="['groupreport:groupProfession:remove']" :disabled="multipleR">移除 </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain size="mini" icon="el-icon-s-management" @click="handlesave" v-hasPermi="['groupreport:groupProfession:save']" :disabled="this.ids.length == 0 || isPass">保存 </el-button>
          </el-col>
          <el-col :span="2">
            <el-button type="warning" plain size="mini" icon="el-icon-upload2" @click="handleExport" v-hasPermi="['groupreport:groupProfession:export']" :disabled="this.ids.length == 0 || isPass">导出 </el-button>
          </el-col>
        </el-row>
        <!-- 表格 -->
        <div class="table-box" ref="tableBox">
          <el-table class="add-table" border v-loading="rightLoading" :data="rightTableList" height="100%" stripe @selection-change="handleSelectionChangeR">
            <el-table-column fixed type="selection" align="center" />
            <el-table-column label="序列" type="index" width="65" align="center" />
...
<script>
import { getListData, removeData, addPerson, loadPersonData, removePersons, saveData } from '@/api/groupreport/health_profession.js'
import addItems from './add'
export default {
  name: 'Group_profession',
  components: { addItems },
  data() {
    return {
      // 日期选择参数
      ...
      if (value.patientcode) {
        ...
        for (let i in this.rightTableList) {
          if (index == this.rightTableList[i].index) {
            ...
            this.rightTableList[i].index = index
