...
<template>
  <div class="add-container">
    <!-- 添加或修改对话框 -->
    <el-dialog :title="title" :visible.sync="open" width="900px" append-to-body :destroy-on-close="true" :close-on-click-modal="false">
      <el-form ref="form" :model="form" :inline="true" label-width="130px" hide-required-asterisk :rules="rules" v-loading="loading">
        <el-form-item label="团体名称" prop="orgName">
          <input-select ref="groupName" :selectData="ptcodeData" :initialValue="form.orgName" selectWidth="255" @change="ptcodeDataChange"></input-select>
        </el-form-item>
        <el-form-item label="样本类型" prop="sampleType">
          <el-input v-model="form.sampleType" readonly style="width: 255px" />
        </el-form-item>
        <el-form-item label="样本名称" prop="sampleName">
          <el-input v-model="form.sampleName" placeholder="请输入" clearable style="width: 255px" />
        </el-form-item>
        <el-form-item label="订单号" prop="orderId">
          <template v-if="!form.groupId || (form.groupId && orderList.length == 0)">
            <el-input v-model="form.orderId" placeholder="请输入订单号后回车搜索" clearable style="width: 255px" @input="orderReset" @keyup.enter.native="orderSearch" />
          </template>
          <template v-else>
            <el-select v-model="form.orderId" placeholder="请选择" clearable style="width: 255px" @change="orderChange" :disabled="orderList.length == 0">
              <el-option v-for="item in orderList" :key="item.id" :label="item.ddh" :value="item.ddh" />
            </el-select>
          </template>
        </el-form-item>
        <el-form-item label="开始登记时间" prop="beginTime">
          <el-date-picker v-model="form.beginTime" style="width: 255px" value-format="yyyy-MM-dd" type="date" :placeholder="form.beginTime"></el-date-picker>
        </el-form-item>
        <el-form-item label="结束登记时间" prop="endTime">
          <el-date-picker v-model="form.endTime" style="width: 255px" value-format="yyyy-MM-dd" type="date" :placeholder="form.endTime"></el-date-picker>
        </el-form-item>
        <el-form-item label="体检团体类型" prop="idOrgclass">
          <el-select v-model="form.idOrgclass" placeholder="请选择" clearable style="width: 255px" disabled>
        ...
        <el-form-item label="上次是否来检" prop="scsflj">
          <el-select v-model="form.scsflj" placeholder="请选择" style="width: 255px">
            <el-option label="否" :value="0" />
            <el-option label="是" :value="1" />
          </el-select>
        </el-form-item>
        <el-form-item label="是否含未检" prop="hasUnchecked">
          <el-checkbox v-model="form.hasUnchecked" style="width: 255px"></el-checkbox>
        </el-form-item>
        <el-form-item label="性别" prop="sex">
          <el-select v-model="form.sex" placeholder="通用" clearable style="width: 255px">
            <el-option label="男" :value="0" />
            <el-option label="女" :value="1" />
          </el-select>
        </el-form-item>
        <el-form-item label="上岗类型" prop="sglx">
          <el-radio-group v-model="form.sglx">
            <el-radio v-for="item in postList" :key="item.value" :label="item.value">{{ item.label }}</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="部门" prop="orgDepart">
          <el-input v-model="form.orgDepart" placeholder="请输入" clearable style="width: 255px" />
        </el-form-item>
        <el-form-item label="分中心" prop="fzxid">
          <el-select v-model="form.fzxid" placeholder="请选择分中心" multiple style="width: 255px">
            <el-option v-for="item in fzxOptions" :key="item.id" :label="item.fzx" :value="item.branchId"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="上次职业健康检查情况" prop="bgfx">
          <el-input v-model="form.bgfx" clearable style="width: 650px" type="textarea" :rows="4" resize="none" />
        </el-form-item>
        <el-form-item label="本次职业健康检查情况" prop="bcbgfx">
          <el-input v-model="form.bcbgfx" readonly style="width: 650px" type="textarea" :rows="4" resize="none" />
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    handleUpdate(id) {
      ...
      getDetails(id).then(({ data }) => {
        ...
        this.form.sglx = Number(data.sglx)
    ...
    // 处理订单号输入
    handleOrderIdInput() {
      const val = this.form.orderId;
      if (val) {
        checkType(val).then(({ data }) => {
          // 如果能找到匹配的订单
          const matchedOrder = this.orderList.find(item => item.ddh == val);
          if (matchedOrder) {
            this.form.ddid = matchedOrder.id;
          }
          this.form.idOrgclass = data.idOrgclass;
        }).catch(() => {
          // 如果checkType调用失败，可能需要清除之前设置的值
          // this.form.ddid = undefined;
          // this.form.idOrgclass = undefined;
        });
      }
    ...
    submitForm() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          form.beginTime = form.beginTime + ' 00:00:00'
          form.endTime = form.endTime + ' 23:59:59'
          form.sglx = form.sglx.toString()
          ...
          form.idOrgclass = undefined
          form.diseaseHealth = 1
