...
<template>
  <div class="app-container flex-direction-column">
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button class="zk-btn-style2" icon="el-icon-edit" size="mini" @click="handleUpdate" v-hasPermi="['system:branch:edit']">详情</el-button>
      </el-col>
    </el-row>
    <div class="table-box">
      <el-table ref="dataList" border v-loading="loading" :data="dataList" height="100%" @selection-change="handleSelectionChange" @row-click="rowClick">
        ...
        <el-table-column label="ip" align="center" prop="address" width="100" show-overflow-tooltip />
        <el-table-column label="主机名" align="center" prop="hostName" width="100" show-overflow-tooltip />
        <el-table-column label="执行时间" align="center" prop="executeTime" width="100" show-overflow-tooltip />
        <el-table-column label="执行结果" align="center" prop="message" width="100" show-overflow-tooltip />
        <el-table-column label="备注" align="center" prop="remark" show-overflow-tooltip />
        <el-table-column label="状态" align="center" width="90" prop="status">
          <template slot-scope="scope">
            <el-tag type="danger" v-if="scope.row.status == -1">更新失败</el-tag>
            <el-tag v-else-if="scope.row.status == 0">待更新</el-tag>
            <el-tag v-else-if="scope.row.status == 1">正在更新</el-tag>
    ...
    <!-- 授权详情 -->
    <version-details ref="versionDetails"></version-details>
  </div>
</template>

<script>
import { getListApi } from '@/api/system/version_control/version_record'
...
export default {
  ...
  created() {
    let branchId = this.$getCookie('cid')
    if (branchId != '2') {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.$refs.versionDetails.showDialog(id)
    },
  },
}
</script>
