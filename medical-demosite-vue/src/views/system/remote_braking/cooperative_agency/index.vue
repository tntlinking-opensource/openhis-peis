...
<template>
  <div class="app-container flex-direction-column">
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button class="zk-btn-style2" icon="el-icon-plus" size="mini" :disabled="single" @click="handleView" v-hasPermi="['system:branch:view']">添加授权</el-button>
      </el-col>
    </el-row>
    <div class="table-box">
      <el-table ref="dataList" border v-loading="loading" :data="dataList" height="100%" @selection-change="handleSelectionChange" @row-click="rowClick">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            ...
            <el-button type="text" size="mini" icon="el-icon-plus" @click="handleView(scope.row)" v-hasPermi="['system:branch:view']">添加授权</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog :title="title" :visible.sync="open" width="860px" class="sub-center report-setting" destroy-on-close append-to-body :close-on-click-modal="false" @close="reset">
      <el-form ref="form" :model="form" :rules="rules" label-width="120px" :inline="true" hide-required-asterisk v-loading="loading">
        ...
        <el-form-item label="排序" prop="seq">
          <el-input type="number" v-model="form.seq" placeholder="请输入排序" style="width: 220px" />
        </el-form-item>
    ...
    <!-- 授权记录 -->
    <accredit-record ref="accreditRecord"></accredit-record>
  </div>
</template>

<script>
import { getListApi, addConfig, removeConfig, ConfigDetails, editConfig } from '@/api/system/remote_braking/cooperative_agency'
import accreditRecord from './accredit_record'
export default {
  ...
  methods: {
    ...
    // 查看授权记录
    handleView(row) {
      let data = this.selection[0] || row
      this.$refs.accreditRecord.showDialog(data)
    },
    /** 提交按钮 */
    submitForm() {
