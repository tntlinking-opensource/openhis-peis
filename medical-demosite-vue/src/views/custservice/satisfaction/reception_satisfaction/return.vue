<template>
  <el-dialog title="前台满意度回访" :visible.sync="open" class="add-items" width="900px" :close-on-click-modal="false"
      append-to-body>
      <el-form v-loading="popLoading" ref="form"  :model="form" :inline="true" label-width="102px" hide-required-asterisk>
        ...
        <el-form-item prop="visitTime"  label="回访时间">
          <el-date-picker  type="date" value-format="yyyy-MM-dd" placeholder="请选择回访时间" v-model="form.visitTime" style="width:308px"></el-date-picker>
        </el-form-item>
...
<script>
import { getDetail,addList ,saveData} from "@/api/custservice/satisfaction/reception_satisfaction.js";
export default {
    data(){
      return{
        //只读
    ...
    methods:{
      ...
      submitForm()
      {
        ...
        this.form.visitTime =  this.form.visitTime + " 00:00:00"
      } else {
      ...
      getNewList(query)
    {
      ...
      getDetail(query).then(response => {
        const id =response.data.Satisfaction.id
        this.form =Object.assign(response.data.Satisfaction,response.data.peispatient)
        this.form.id=id
