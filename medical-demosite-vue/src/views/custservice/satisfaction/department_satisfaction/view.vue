<template>
  <el-dialog title="科室满意度回访" :visible.sync="open" class="add-items" width="900px" :close-on-click-modal="false"
    append-to-body>
    <el-form v-loading="popLoading" ref="form" :model="form" :inline="true" label-width="102px" hide-required-asterisk>
      ...
      <el-form-item prop="visitTime" label="回访时间">
        <el-date-picker type="date" placeholder="请选择回访时间" value-format="yyyy-MM-dd" v-model="form.visitTime"
...
<script>
import { detailData,saveData } from '@/api/custservice/satisfaction/department_satisfaction.js'
export default {
  ...
  methods: {
    ...
    getNewList(query) {
      ...
      this.open = true;
      this.reset();
      detailData(query).then(response => {
        //判断是否存在此对象主要赋值对象
