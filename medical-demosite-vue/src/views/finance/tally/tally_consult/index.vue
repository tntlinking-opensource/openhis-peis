<!-- 记账查询 开发人：麦沃德科技暴雨/矢北 -->
<template>
  <div class="app-container flex-direction-column tally-consult">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" class="no-margin-bottom">
      ...
      <el-form-item label="记账人" prop="jzr">
        <el-input v-model="queryParams.jzr" placeholder="请输入输入码" clearable style="width: 180px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="记账单位" prop="jzdw">
        <el-input v-model="queryParams.jzdw" placeholder="请输入输入码" clearable style="width: 180px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="结算状态" prop="js">
        <el-select style="width: 180px" v-model="queryParams.js" clearable>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain size="mini" icon="el-icon-plus" :disabled="single" @click="handleRefund" v-hasPermi="['finance:tally:tallyConsult:refund']">结算 </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain size="mini" icon="el-icon-edit" @click="handleExport" v-hasPermi="['finance:tally:tallyConsult:export']">导出 </el-button>
      </el-col>
    </el-row>
    <!-- 表格 -->
    <div class="table-box">
      <el-table ref="tableList" border v-loading="loading" :data="tableList" height="100%" stripe @selection-change="handleSelectionChange" @row-click="rowClick">
        ...
        <el-table-column label="记账未结" prop="paid" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            {{ scope.row.paid ? Number(scope.row.paid).toFixed(2) : '0.00' }}
          </template>
        </el-table-column>
        <el-table-column label="记账合计" prop="jzje" align="center" show-overflow-tooltip />
        <el-table-column label="记账人" prop="jzr" align="center" show-overflow-tooltip />
        <el-table-column label="记账单位" prop="jzdw" align="center" show-overflow-tooltip />
        <el-table-column label="收费员" prop="feechargerName" align="center" show-overflow-tooltip />
        <el-table-column label="审批人" prop="spr" align="center" show-overflow-tooltip />
        <el-table-column label="记账日期" prop="jzDate" align="center" show-overflow-tooltip />
        <el-table-column label="结算金额" prop="ypid" align="center" show-overflow-tooltip />
        <el-table-column label="开单医师" prop="doctorapply" align="center" show-overflow-tooltip />
        <el-table-column label="结算状态" prop="jszt" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
...
import settlement from '../tally_refund/settlement.vue'
...
export default {
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('/finance/tallyQuery/export', this.queryParams, `记账信息列表.xlsx`)
    },
  },
}
</script>
<style scoped>
.tally-consult /deep/ .el-table__header-wrapper .el-checkbox {
  display: none;
}
</style>
