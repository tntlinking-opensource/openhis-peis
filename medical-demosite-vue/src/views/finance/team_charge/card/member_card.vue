  ...
  <el-dialog title="会员卡积分调整" :visible.sync="open" width="750px" append-to-body :close-on-click-modal="false" destroy-on-close @close="cancel">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" label-width="90px" v-loading="loadingCard" :rules="rules" hide-required-asterisk>
      <el-form-item label="会员卡号" prop="patientcardno">
        <el-input ref="cardno" v-model="queryParams.patientcardno" :readonly="readOnly" placeholder="请输入会员卡号" clearable style="width: 250px" @keyup.enter.native="handleCardNum(1)"></el-input>
      </el-form-item>
      ...
      <el-form-item label="充值积分" prop="realityMoney">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        realityMoney: [{ required: true, message: '充值积分不能为空', trigger: 'change' }],
      },
      // 退款时只读
      readOnly: false,
  ...
  methods: {
    dialogShow(info, ids) {
      ...
      this.queryParams = {
        ...
        realityMoney: (0 - ids.moneyamountpaid).toFixed(2),
        cardId: undefined,
        kkfs: 2,
        memotext: '消费',
      }
    ...
    changerealityMoney() {
      if (this.queryParams.realityMoney) {
        this.$nextTick(() => {
          ...
          this.queryParams.shouldMoney = 0 - Number(this.queryParams.realityMoney).toFixed(2)
        })
      }
    },
    // 查询会员卡
