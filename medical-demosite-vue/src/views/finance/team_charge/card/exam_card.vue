  ...
  <el-dialog title="体检卡消费" :visible.sync="open" width="780px" append-to-body :close-on-click-modal="false" @close="cancel" destroy-on-close>
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" label-width="100px" :rules="rules" hide-required-asterisk v-loading="loading">
      <el-form-item label="体检卡号" prop="cardId">
        <el-input ref="cardno" v-model="queryParams.cardId" placeholder="请输入体检卡号" clearable :readonly="readOnly" style="width: 250px" @keyup.enter.native="handleCardNum"></el-input>
      </el-form-item>
      ...
      <el-form-item label="分中心" prop="fzx">
        <input-select :selectData="selectData" @change="selectChange" :initialValue="queryParams.fzx"></input-select>
      </el-form-item>
      ...
      <el-form-item label="结算费" prop="shouldMoney">
        <el-input v-model="queryParams.shouldMoney" readonly style="width: 250px"> </el-input>
      </el-form-item>
      <el-form-item label="消费金额" prop="realityMoney">
        <el-input type="number" v-model="queryParams.realityMoney" readonly style="width: 250px" @blur="changerealityMoney"> </el-input>
      </el-form-item>
...
<script>
import { getCardData } from '@/api/reception/proceeds.js'
import { saveOrUpdateFee } from '@/api/finance/team_charge.js'
  ...
  methods: {
    dialogShow(info, ids) {
      ...
      this.queryParams = {
        ...
        fzx: undefined,
        idPayway: '7',
        shouldMoney: ids.moneyamountpaid.toFixed(2),
        realityMoney: ids.moneyamountpaid.toFixed(2),
        consumeType: '0',
        kkfs: 1,
        memotext: undefined,
      }
      if (ids.id) {
        ...
        this.queryParams.shouldMoney = 0 - this.queryParams.shouldMoney
        this.queryParams.realityMoney = 0 - this.queryParams.realityMoney
        this.handleCardNum()
      }
      this.open = true
      this.$nextTick(() => {
    ...
    // 充值积分保留1位小数
    changerealityMoney() {
      if (this.queryParams.realityMoney) {
        this.$nextTick(() => {
          this.queryParams.realityMoney = Number(this.queryParams.realityMoney).toFixed(2)
          this.queryParams.shouldMoney = Number(this.queryParams.realityMoney).toFixed(2)
        })
      }
    },
    // 查询体检卡号
        ...
        if (valid) {
          ...
          this.$confirm('确定要操作?', '提示')
            .then(() => {
              ...
              queryParams.realityMoney = 0 - queryParams.realityMoney
