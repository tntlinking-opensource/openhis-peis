...
public class ZySummaryController extends BaseController {
     ...
     * @param zySummary       查询条件
     * @return 所有数据
     */
    @GetMapping("/page")
    //@PreAuthorize("@ss.hasPermi('::list')")
    @ApiOperation(value = "分页查询", notes = "分页查询JC职业病检查结论")
    public R<IPage<ZySummary>> getPage(PageParamSimple pageParamSimple, ZySummary zySummary) {
        PageParam<ZySummary> page = mapperFacade.map(pageParamSimple, PageParam.class);
        return R.ok(this.zySummaryService.getPage(page, zySummary));
    }
    ...
    @ApiOperationSupport(ignoreParameters = {"zySummary.id"})
    public R insert(@RequestBody ZySummary zySummary) {
        return R.toResult(this.zySummaryService.save(zySummary));
    }
    ...
    public R update(@RequestBody ZySummary zySummary) {
        return R.toResult(this.zySummaryService.updateById(zySummary));
    }
    ...
    public R delete(@PathVariable List<String> ids) {
        return R.toResult(this.zySummaryService.removeByIds(ids));
    }
}
