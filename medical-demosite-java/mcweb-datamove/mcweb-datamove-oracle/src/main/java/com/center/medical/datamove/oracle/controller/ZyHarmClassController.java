...
public class ZyHarmClassController extends BaseController {
     ...
     * @param zyHarmClass     查询条件
     * @return 所有数据
     */
    @GetMapping("/page")
    //@PreAuthorize("@ss.hasPermi('::list')")
    @ApiOperation(value = "分页查询", notes = "分页查询")
    public R<IPage<ZyHarmClass>> getPage(PageParamSimple pageParamSimple, ZyHarmClass zyHarmClass) {
        PageParam<ZyHarmClass> page = mapperFacade.map(pageParamSimple, PageParam.class);
        return R.ok(this.zyHarmClassService.getPage(page, zyHarmClass));
    }
    ...
    @ApiOperationSupport(ignoreParameters = {"zyHarmClass.id"})
    public R insert(@RequestBody ZyHarmClass zyHarmClass) {
        return R.toResult(this.zyHarmClassService.save(zyHarmClass));
    }
    ...
    public R delete(@PathVariable List<String> ids) {
        return R.toResult(this.zyHarmClassService.removeByIds(ids));
    }
}
