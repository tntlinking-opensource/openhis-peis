package com.center.medical.datamove.oracle.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.center.medical.datamove.oracle.dao.TempFeeitemMapper;
import com.center.medical.datamove.oracle.bean.model.TempFeeitem;
import com.center.medical.datamove.oracle.service.TempFeeitemService;
import org.springframework.stereotype.Service;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.center.medical.common.utils.page.PageParam;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;

/**
 * (TempFeeitem)服务实现类
 *
 * @author ay
 * @since 2023-07-18 09:25:16
 */
@Slf4j
@Service("tempFeeitemService")
@RequiredArgsConstructor
public class TempFeeitemServiceImpl extends ServiceImpl<TempFeeitemMapper, TempFeeitem> implements TempFeeitemService {

    private final TempFeeitemMapper tempFeeitemMapper;

    /**
     * 分页查询[]列表
     *
     * @param page  分页参数
     * @param param TempFeeitem查询参数
     * @return 分页数据
     */
    @Override
    public IPage<TempFeeitem> getPage(PageParam<TempFeeitem> page, TempFeeitem param) {
        return tempFeeitemMapper.getPage(page, param);
    }

    /**
     * 根据主键id获取记录详情
     *
     * @param id 主键id
     * @return 详情信息
     */
    @Override
    public TempFeeitem getInfoById(String id) {
        return tempFeeitemMapper.getInfoById(id);
    }

}


