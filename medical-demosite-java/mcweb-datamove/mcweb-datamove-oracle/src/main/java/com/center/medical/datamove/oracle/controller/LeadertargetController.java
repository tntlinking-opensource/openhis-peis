...
public class LeadertargetController extends BaseController {
     ...
     * @param leadertarget    查询条件
     * @return 所有数据
     */
    @GetMapping("/page")
    //@PreAuthorize("@ss.hasPermi('::list')")
    @ApiOperation(value = "分页查询", notes = "分页查询领导目标表")
    public R<IPage<Leadertarget>> getPage(PageParamSimple pageParamSimple, Leadertarget leadertarget) {
        PageParam<Leadertarget> page = mapperFacade.map(pageParamSimple, PageParam.class);
        return R.ok(this.leadertargetService.getPage(page, leadertarget));
    }
    ...
    @ApiOperationSupport(ignoreParameters = {"leadertarget.id"})
    public R insert(@RequestBody Leadertarget leadertarget) {
        return R.toResult(this.leadertargetService.save(leadertarget));
    }
    ...
    public R update(@RequestBody Leadertarget leadertarget) {
        return R.toResult(this.leadertargetService.updateById(leadertarget));
    }
    ...
    public R delete(@PathVariable List<String> ids) {
        return R.toResult(this.leadertargetService.removeByIds(ids));
    }
}
