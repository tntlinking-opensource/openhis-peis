package com.center.medical.app.handler;
...
@Slf4j
@Component
public class OpenApiHandler implements HandlerInterceptor {
    @Resource
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (Objects.isNull(aesEncryptDto) || StringUtils.isBlank(aesEncryptDto.getEncryptData()) || StringUtils.isBlank(aesEncryptDto.getEncryptData())) {
            //log.info("5、AES解密后参数为空！");
            errorResponse(response, HttpServletResponse.SC_BAD_REQUEST, "请求参数不合法！");
            return false;
//            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "请求参数不合法！");
        }
        ...
        //验证授权码是否有效
        AuthCode authCodeItem = authCodeService.getOne(new LambdaQueryWrapper<AuthCode>().eq(AuthCode::getAuthCode, authCode)
                .eq(AuthCode::getAuthFlag, flag).eq(AuthCode::getStatus, 1).gt(AuthCode::getExpiryDate, new Date()));
    ...
    private void errorResponse(HttpServletResponse response, Integer code, String msg) {
        ...
        AppResponse<Object> fail = AppResponseEntity.fail(code, msg);
