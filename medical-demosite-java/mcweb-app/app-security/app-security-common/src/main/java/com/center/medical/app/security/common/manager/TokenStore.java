package com.center.medical.app.security.common.manager;
...
import com.center.medical.app.common.constants.OauthCacheNames;
import com.center.medical.app.common.enums.AppHttpStatus;
import com.center.medical.app.common.exception.AppBindException;
import com.center.medical.app.common.serializer.redis.KryoRedisSerializer;
...
public class TokenStore {
    ...
    public UserInfoInTokenBO getUserInfoByAccessToken(String accessToken, boolean needDecrypt) {
        if (StrUtil.isBlank(accessToken)) {
            throw new AppBindException(AppHttpStatus.UNAUTHORIZED, "accessToken is blank");
        }
        ...
        if (userInfoInTokenBO == null) {
            throw new AppBindException(AppHttpStatus.UNAUTHORIZED, "accessToken 已过期");
        }
    ...
    public TokenInfoBO refreshToken(String refreshToken) {
        if (StrUtil.isBlank(refreshToken)) {
            throw new AppBindException(AppHttpStatus.UNAUTHORIZED, "refreshToken is blank");
        }
        ...
        if (StrUtil.isBlank(accessToken)) {
            throw new AppBindException(AppHttpStatus.UNAUTHORIZED, "refreshToken 已过期");
        }
    ...
    private String decryptToken(String data) {
        ...
        try {
            ...
            if (System.currentTimeMillis() - createTokenTime > expiresIn * second) {
                throw new AppBindException(AppHttpStatus.UNAUTHORIZED, "token error");
            }
        } catch (Exception e) {
            throw new AppBindException(AppHttpStatus.UNAUTHORIZED, "token error");
        }
