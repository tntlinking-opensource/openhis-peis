...
public class FtpUtil {
     ...
     * @return
     */
    public static Boolean upload(String destPath, File file) {
        String config = RedisUtil.get(Constants.FTP_CONFIG);
        FtpConfig ftpConfig = JSONUtil.toBean(config, FtpConfig.class);
        log.info("FTP配置信息11：{}", JSONUtil.toJsonStr(ftpConfig));
        try (
            ...
            return ftp.upload(destPath, file);
...
//        log.info("上传文件:{}、{}、{}、{}", destPath, fileName, file, isFtp);
        if (isFtp){
            String config = RedisUtil.get(Constants.FTP_CONFIG);
            FtpConfig ftpConfig = JSONUtil.toBean(config, FtpConfig.class);
//        log.info("FTP配置信息：{}", JSONUtil.toJsonStr(ftpConfig));
            try (
                    Ftp ftp = new Ftp(ftpConfig, FtpMode.Passive);
            ) {
                ftp.mkDirs(destPath);
                boolean upload = ftp.upload(destPath, fileName, file);
//                log.info("ftp上传文件:{}", upload);
                if (!upload) {
                    // TODO wait 上传失败，记录失败记录
