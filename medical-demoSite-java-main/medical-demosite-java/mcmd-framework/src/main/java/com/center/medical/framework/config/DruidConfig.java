...
import com.center.medical.framework.config.properties.DruidProperties;
import com.center.medical.framework.datasource.DynamicDataSource;
...
public class DruidConfig {
    ...
    @Bean
    @ConfigurationProperties("spring.datasource.druid.zhonghe")
    @ConditionalOnProperty(prefix = "spring.datasource.druid.zhonghe", name = "enabled", havingValue = "true")
    public DataSource zhongheDataSource(DruidProperties druidProperties) {
        DruidDataSource dataSource = DruidDataSourceBuilder.create().build();
        return druidProperties.dataSource(dataSource);
    }
    ...
    public DynamicDataSource dataSource(DataSource masterDataSource) {
        ...
        setDataSource(targetDataSources, DataSourceType.OLDORACLE.name(), "oldoracleDataSource");
        setDataSource(targetDataSources, DataSourceType.SLAVE2.name(), "slave2DataSource");
        setDataSource(targetDataSources, DataSourceType.APPLET.name(), "appletDataSource");
        setDataSource(targetDataSources, DataSourceType.HONGQIAO.name(), "hongqiaoDataSource");
        setDataSource(targetDataSources, DataSourceType.HONGQIAOSLAVE.name(), "hongqiaoslaveDataSource");
        setDataSource(targetDataSources, DataSourceType.RUIMEI.name(), "ruimeiDataSource");
        setDataSource(targetDataSources, DataSourceType.RUIMEISLAVE.name(), "ruimeislaveDataSource");
        setDataSource(targetDataSources, DataSourceType.JINWEI.name(), "jinweiDataSource");
        setDataSource(targetDataSources, DataSourceType.RILIN.name(), "rilinDataSource");
        setDataSource(targetDataSources, DataSourceType.ZHONGHE.name(), "zhongheDataSource");
