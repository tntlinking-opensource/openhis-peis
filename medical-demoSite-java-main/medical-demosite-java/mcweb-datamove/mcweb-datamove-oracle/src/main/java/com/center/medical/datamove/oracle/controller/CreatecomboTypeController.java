...
@Api(tags = "最小套餐分类")
@RequestMapping("createcomboType")
public class CreatecomboTypeController extends BaseController {
     ...
     * @param createcomboType 查询条件
     * @return 所有数据
     */
    @GetMapping("/page")
    //@PreAuthorize("@ss.hasPermi('::list')")
    @ApiOperation(value = "分页查询", notes = "分页查询最小套餐分类")
    public R<IPage<CreatecomboType>> getPage(PageParamSimple pageParamSimple, CreatecomboType createcomboType) {
        PageParam<CreatecomboType> page = mapperFacade.map(pageParamSimple, PageParam.class);
        return R.ok(this.createcomboTypeService.getPage(page, createcomboType));
    }
