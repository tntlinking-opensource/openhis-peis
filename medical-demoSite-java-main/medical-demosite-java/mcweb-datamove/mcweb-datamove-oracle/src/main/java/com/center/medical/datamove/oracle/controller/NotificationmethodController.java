...
/**
 * 通知方式（领取方式）表(Notificationmethod)接口
 *
 * @author ay
...
public class NotificationmethodController extends BaseController {
     ...
     * @param notificationmethod 查询条件
     * @return 所有数据
     */
    @GetMapping("/page")
    //@PreAuthorize("@ss.hasPermi('::list')")
    @ApiOperation(value = "分页查询", notes = "分页查询通知方式（领取方式）表")
    public R<IPage<Notificationmethod>> getPage(PageParamSimple pageParamSimple, Notificationmethod notificationmethod) {
        PageParam<Notificationmethod> page = mapperFacade.map(pageParamSimple, PageParam.class);
        return R.ok(this.notificationmethodService.getPage(page, notificationmethod));
    }
    ...
    public R insert(@RequestBody Notificationmethod notificationmethod) {
        return R.toResult(this.notificationmethodService.save(notificationmethod));
    }
    ...
    public R update(@RequestBody Notificationmethod notificationmethod) {
        return R.toResult(this.notificationmethodService.updateById(notificationmethod));
    }
    ...
    public R delete(@PathVariable List<String> ids) {
        return R.toResult(this.notificationmethodService.removeByIds(ids));
    }
}
