...
/**
 * 主表记录体检者是否来检，如果来检，显示来检时间，便于筛选，在预约来检未检列表中显示来检时间且是否已检为未检的做出标记，在预约来检未检回访操作时将该状态、来检时间重置，以此进行无限循环(MdAdvanceVisitWrite)接口
 *
 * @author ay
...
@RequestMapping("mdAdvanceVisitWrite")
public class MdAdvanceVisitWriteController extends BaseController {
    /**
     * 服务对象
     */
    private final MdAdvanceVisitWriteService mdAdvanceVisitWriteService;
     ...
     * @param mdAdvanceVisitWrite 查询条件
     * @return 所有数据
     */
    @GetMapping("/page")
    ...
    public R<IPage<MdAdvanceVisitWrite>> getPage(PageParamSimple pageParamSimple, MdAdvanceVisitWrite mdAdvanceVisitWrite) {
        PageParam<MdAdvanceVisitWrite> page = mapperFacade.map(pageParamSimple, PageParam.class);
        return R.ok(this.mdAdvanceVisitWriteService.getPage(page, mdAdvanceVisitWrite));
    }
    ...
    public R<MdAdvanceVisitWrite> selectOne(@PathVariable String id) {
        return R.ok(this.mdAdvanceVisitWriteService.getInfoById(id));
    }

    /**
     * 新增数据
     *
     * @param mdAdvanceVisitWrite 实体对象
     * @return 新增结果
     */
    @PostMapping
    ...
    @ApiOperationSupport(ignoreParameters = {"mdAdvanceVisitWrite.id"})
    public R insert(@RequestBody MdAdvanceVisitWrite mdAdvanceVisitWrite) {
        return R.toResult(this.mdAdvanceVisitWriteService.save(mdAdvanceVisitWrite));
    }
    ...
    public R update(@RequestBody MdAdvanceVisitWrite mdAdvanceVisitWrite) {
        return R.toResult(this.mdAdvanceVisitWriteService.updateById(mdAdvanceVisitWrite));
    }
    ...
    public R delete(@PathVariable List<String> ids) {
        return R.toResult(this.mdAdvanceVisitWriteService.removeByIds(ids));
    }
}
