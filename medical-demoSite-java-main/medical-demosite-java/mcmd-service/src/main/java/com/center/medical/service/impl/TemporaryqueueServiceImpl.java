package com.center.medical.service.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.center.medical.dao.TemporaryqueueMapper;
import com.center.medical.bean.model.Temporaryqueue;
import com.center.medical.service.TemporaryqueueService;
import org.springframework.stereotype.Service;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.center.medical.common.utils.page.PageParam;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;

/**
 * temporaryqueue(Temporaryqueue)表服务实现类
 *
 * @author 路飞船长
 * @since 2022-11-08 18:35:01
 */
@Slf4j
@Service("temporaryqueueService")
@RequiredArgsConstructor
public class TemporaryqueueServiceImpl extends ServiceImpl<TemporaryqueueMapper, Temporaryqueue> implements TemporaryqueueService {

    private final TemporaryqueueMapper temporaryqueueMapper;

    /**
     * 分页查询[temporaryqueue]列表
     *
     * @param page  分页参数
     * @param param Temporaryqueue查询参数
     * @return 分页数据
     */
    @Override
    public IPage<Temporaryqueue> getList(PageParam<Temporaryqueue> page, Temporaryqueue param) {
        return temporaryqueueMapper.getList(page, param);
    }

    /**
     * 根据主键id获取记录详情
     *
     * @param id id主键
     */
    @Override
    public Temporaryqueue getInfoById(String id) {
        return temporaryqueueMapper.getInfoById(id);
    }

}

