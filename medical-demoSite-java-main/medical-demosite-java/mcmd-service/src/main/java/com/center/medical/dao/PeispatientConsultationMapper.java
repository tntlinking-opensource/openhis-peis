package com.center.medical.dao;

import com.baomidou.mybatisplus.core.mapper.BaseMapper;
import com.center.medical.bean.model.PeispatientConsultation;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.center.medical.common.utils.page.PageParam;
import org.apache.ibatis.annotations.Param;

/**
 * 职业问诊(PeispatientConsultation)表数据库访问层
 *
 * @author 路飞船长
 * @since 2022-11-08 18:34:27
 */
public interface PeispatientConsultationMapper extends BaseMapper<PeispatientConsultation> {

    /**
     * 分页查询[职业问诊]列表
     *
     * @param page  分页参数
     * @param param PeispatientConsultation查询参数
     * @return 分页数据
     */
    IPage<PeispatientConsultation> getList(PageParam<PeispatientConsultation> page, @Param("param") PeispatientConsultation param);

    /**
     * 根据主键id获取记录详情
     *
     * @param id id主键
     */
    PeispatientConsultation getInfoById(@Param("id") String id);

}
